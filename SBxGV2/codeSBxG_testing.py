...
def enc(fileToSave):
    ...
    while 1:
        for i in range(0, 100):     #get content of first 100 blocks
            ...
            secondHalf = rsc.encode(buffer[213:None])  # add second half to variable
...
def dec():
    ...
    while True:
        ...
        if(counter>200000000):
            tempFile = open('recovered_test.txt', 'wb')
            tempFile.write('A'.encode())
            tempFile.close()
            exit()
        #-----------------------only for testing
                    ...
                    if option == '1':
                        ...
                        try:
                            while blocksFound < bam:            #search blocks
                                ...
                                if buffer[0:2] == 'SB'.encode() and buffer[2:8] == fUid:
                        ...
                        for i in range(0, bam):
                            for y in range(0, 100):
                                ...
                                blockCollectionEncoded.append(buffer)
                                ...
                                if i == bam - 1:  # check if its last 100 Blocks
                                    for byte_sequence in range(0, 100):
                                        recovered_block += blockCollectionEncoded[byte_sequence][
                                                           12 + block * 5:12 + (block + 1) * 5]

                                    if byte_amount - counter <= 436:
